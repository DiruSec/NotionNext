...
const NavBar = props => {
  ...
  let links = [
    { id: 2, name: locale.NAV.RSS, to: `${siteConfig('SUB_PATH') || ''}/feed`, show: siteConfig('ENABLE_RSS') && siteConfig('NOBELIUM_MENU_RSS', null, CONFIG), target: '_blank' },
    { icon: 'fas fa-search', name: locale.NAV.SEARCH, to: `${siteConfig('SUB_PATH') || ''}/search`, show: siteConfig('NOBELIUM_MENU_SEARCH', null, CONFIG) },
    { icon: 'fas fa-archive', name: locale.NAV.ARCHIVE, to: `${siteConfig('SUB_PATH') || ''}/archive`, show: siteConfig('NOBELIUM_MENU_ARCHIVE', null, CONFIG) },
    { icon: 'fas fa-folder', name: locale.COMMON.CATEGORY, to: `${siteConfig('SUB_PATH') || ''}/category`, show: siteConfig('NOBELIUM_MENU_CATEGORY', null, CONFIG) },
    { icon: 'fas fa-tag', name: locale.COMMON.TAGS, to: `${siteConfig('SUB_PATH') || ''}/tag`, show: siteConfig('NOBELIUM_MENU_TAG', null, CONFIG) }
  ]
  if (customNav) {
